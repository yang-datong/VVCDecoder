...
12. [2026-03-16 19:06:19 CST] 继续对照本地 `vvdec-3.2.0/` 的 `D_CABAC + D_SYNTAX + D_HEADER` trace 后，本轮把阻塞点从“首个 split bin 位置不明”继续推进到了 **首个 leaf CU 之后的 TU/残差子路径**。具体实现上，一方面新增了最小 `VvcCtuPrefixProbe`，先把首个 CTU 在 `coding_tree()` 之前的 `SAO + luma ALF` 前缀消费掉，并把 slice header 里后续前缀要用到的 `num_alf_aps_ids_luma / alf_cb,cr / alf_cc_cb,cr` 等字段接入了解析状态；另一方面，给自写 `VvcCabacContextModel` 补上了 `vvdec Contexts.cpp` 第 4 行 window-size 初始化（`setLog2WindowSize()`），不再只使用前三行 `initId`。在此基础上，`demo.h266` 当前已经能在默认 `payload_rbsp=7` 主路径上稳定复现出与 `vvdec` 一致的首个 CTU 前缀与分块结果：`first_ctu_prefix_bins=20`、`first_ctu_sao_bins=18`、`first_ctu_alf_bins=2`、`first_ctu_sao_y=band`、`first_ctu_sao_y_off={0,1,1,1}`、`first_ctu_sao_y_band=4`、`first_ctu_sao_c=off`、`first_ctu_alf_y=1`、`first_ctu_alf_t=1`，并且首个 split 路径终于从长期错误的 `N` 对齐到了 `vvdec` 一致的 `Q>Q>Q>N`，首个 leaf 也回到了真实的 `8x8`。这说明当前第一优先级已经从“slice_data 起点 / split_flag 错位”切换为：**继续把 `split` 之后的首个 leaf CU 语法探针对齐到真实 8x8 路径；当前主程序已经能到达 `first_cu_luma=1 / first_cu_src=mpm[1]`，但随后在 `transform_tree` 侧撞上了新的更具体阻塞点 `transform_probe_err=Chroma BDPCM syntax is not yet handled by transform tree probe`**。换句话说，下一步应优先补齐这条真实 `8x8` leaf 上的 chroma BDPCM / TU 前缀分支处理，再继续向 `residual / reconstruction` 推进。
